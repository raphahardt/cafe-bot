...
module.exports = function secureRandomNumber(minimum, maximum) {
    return new Promise((resolve, reject) => {
        if (crypto == null || crypto.randomBytes == null) {
            reject(new Error("No suitable random number generator available. Ensure that your runtime is linked against OpenSSL (or an equivalent) correctly."));
        }

        if (minimum == null) {
            reject(new Error("You must specify a minimum value."));
        }

        if (maximum == null) {
            reject(new Error("You must specify a maximum value."));
        }

        if (minimum % 1 !== 0) {
            reject(new Error("The minimum value must be an integer."));
        }

        if (maximum % 1 !== 0) {
            reject(new Error("The maximum value must be an integer."));
        }

        if (!(maximum > minimum)) {
            reject(new Error("The maximum value must be higher than the minimum value."));
        }
        ...
        if (minimum < -9007199254740991 || minimum > 9007199254740991) {
            reject(new Error("The minimum value must be inbetween MIN_SAFE_INTEGER and MAX_SAFE_INTEGER."));
        }

        if (maximum < -9007199254740991 || maximum > 9007199254740991) {
            reject(new Error("The maximum value must be inbetween MIN_SAFE_INTEGER and MAX_SAFE_INTEGER."));
        }

        let range = maximum - minimum;

        if (range < -9007199254740991 || range > 9007199254740991) {
            reject(new Error("The range between the minimum and maximum value must be inbetween MIN_SAFE_INTEGER and MAX_SAFE_INTEGER."));
        }

        let {bitsNeeded, bytesNeeded, mask} = calculateParameters(range);

        crypto.randomBytes(bytesNeeded, (err, randomBytes) => {
            if (err) {
                reject(err);
                return;
            }
